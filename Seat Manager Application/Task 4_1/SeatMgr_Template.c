 ...
 * Generated on 3-30-2024 11:43 AM
 *
...
void SeatManager_AutoHeight (void)
{
	...
	SensorPositionType Arg_;
	SensorWeightType   Weight;
	boolean            AdjustMotor ;

	/* Server Call Points */
	
	status = Rte_Call_rpHeightSensor_GetPosition(&Arg_);
...
void SeatManager_AutoIncline (void)
{
	...
	status = Rte_Call_rpWieghtSensor_GetWeight(&Weight);
	status = Rte_Call_rpInclineSensor_GetPosition(&Arg_);
	AdjustMotor = IsMotorAdjustNeeded( Position , Weight , &Step ) ;
	if(AdjustMotor)
	{	
...
void SeatManager_AutoSlide (void)
{
	...
	status = Rte_Call_rpWieghtSensor_GetWeight(&Weight);
	status = Rte_Call_rpSlideSensor_GetPosition(&Arg_);
	AdjustMotor = IsMotorAdjustNeeded( Position , Weight , &Step ) ;
	if(AdjustMotor)
	{
